...
public class Main extends JPanel implements java.awt.event.MouseListener, java.awt.event.ActionListener
{
    ...
    public void mainMenuGraphic(Graphics g) {
        ...
        if (timer / 1000000000L > 100L) {å
            text.setColor(new Color(0, 153, 51));
        } else if ((timer / 1000000000L < 100L) && (timer / 1000000000L > 25L)) {
            text.setColor(Color.ORANGE);
        } else if (timer / 1000000000L < 25L) {
            text.setColor(new Color(153, 0, 0));
        }
        text.fillRect(0, 0, 800, 800);
    ...
    public void actualMainMenuGraphic(Graphics g) {
        Graphics2D text = (Graphics2D)g;
    ...
    public void highscoreMenuGraphic(Graphics g) {
        Graphics2D text = (Graphics2D)g;
        Font font = getFont("font/direktora.ttf", 90);
        Font questionFont = getFont("font/direktorlight.ttf", 40);
        Font buttonFont = getFont("font/direktor.ttf", 48);
    ...
    public void mouseClicked(MouseEvent e)
    {
        ...
        label392:
        if ((menu == 3) &&
                (collision(e.getX(), e.getY(), 1, 1, 225, 545, 350, 75))) {
            menu = 0;
        }
    }
    ...
    public void paintComponent(Graphics g)
    {
        ...
        object.setRenderingHint(java.awt.RenderingHints.KEY_ANTIALIASING, java.awt.RenderingHints.VALUE_ANTIALIAS_ON);
        if (menu == 0) {
            actualMainMenuGraphic(object);
        } else if (menu == 1) {
            mainMenuGraphic(g);
        } else if (menu == 2) {
            optionMenuGraphic(g);
    ...
    public void run() {
        ...
        if (playing == 1) {
            ...
            if (timer / 1000000000L > 100L) {
                frame.setBackground(new Color(0, 153, 51));
            }
            else if ((timer / 1000000000L < 100L) && (timer / 1000000000L > 25L)) {
    ...
    {
        Shape Main = new java.awt.geom.Rectangle2D.Float(x, y, width, height);
        Shape Enemy = new java.awt.geom.Rectangle2D.Float(x2, y2, width2, height2);
    ...
    {
        Area mainArea = new Area(a);
        mainArea.intersect(new Area(b));
        return !mainArea.isEmpty();
    }
    ...
    public static Font getFont(String name, int size) {
        try {
            java.io.InputStream is = run.getClass().getResourceAsStream(name);
